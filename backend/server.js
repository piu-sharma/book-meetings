...
fs.createReadStream('users.csv')
    .pipe(csv())
    .on('data', (row) => {
        users[row.email.trim()] = { password: row.password.trim(), role: row.role.trim() };
    });
...
app.delete('/bookings/:bookingId', authenticate, authorizeAdmin, (req, res) => {
    ...
    // biome-ignore lint/complexity/noForEach: <explanation>
        wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify({ message: 'Booking deleted', booking: deletedBooking }));
        }
    });
...
app.get('/my-bookings', authenticate, (req, res) => {
    const userBookings = bookings.filter((booking) => booking.bookedBy === req.user.email);
...
app.get('/booking-series', authenticate, authorizeAdmin, (req, res) => {
    ...
    // biome-ignore lint/complexity/noForEach: <explanation>
    bookings
        .filter((booking) => new Date(booking.date) >= new Date(startDate) && new Date(booking.date) <= new Date(endDate))
        .forEach((booking) => {
